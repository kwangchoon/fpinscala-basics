...
enum LazyList[+A]:
  ...
  // Notice the second argument of `f`.
  ...
  // chapter 3), and `zipAll`. The `zipAll` function should continue the traversal
  // as long as either list has more elements—it uses Option to indicate whether
  // each list has been exhausted.
  ...
  // Exercise 5.15
  // Implement `tails` using `unfold`. For a given LazyList, tails returns the
  // LazyList of suffixes of the input sequence, starting with the original
  // LazyList. For example, given LazyList(1,2,3), it would return
  // LazyList(LazyList(1,2,3), LazyList(2,3), LazyList(3), LazyList()).
  ...
  // `foldRight` that returns a stream of the intermediate results. For
  // example:
  //
...
object LazyList:
  ...
  // infinite LazyList of a given value.
  ...
  // Write a more general stream-building function called `unfold`. It takes
  // an initial state, and a function for producing both the next state and
  // the next value in the generated stream.
  // The `unfold` function is an example of what’s sometimes called a
  // corecursive function. Whereas a recursive function consumes data, a
  // corecursive function produces data.
